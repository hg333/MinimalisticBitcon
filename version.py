...
#payload
version = (int(70001)).to_bytes(4,byteorder="little",signed=True)
...
addr_recv_ip = bytes.fromhex("50.116.42.61".encode("utf-8").hex() + ("00")*4)
...
start_height = (int(0)).to_bytes(4,byteorder="little",signed=True)
relay = b'0x00'
...
# prinpytht(checksum, hashlib.sha256(nonce).digest().hex())
print(final_msg.hex())